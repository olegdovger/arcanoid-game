...
void BallSpeedSystem::update(float deltaTime, ECSManager& ecs)
{
    ...
    for (Entity entity : entities)
    {
        ...
        if (gameTime - lastSpeedIncreaseTime >= GameState::BALL_SPEED_INCREASE_INTERVAL)
        {
            // Calculate new speed multiplier
            float newMultiplier = speedMultiplier * GameState::BALL_SPEED_MULTIPLIER;
            
            // Cap at maximum speed
            if (newMultiplier <= GameState::BALL_MAX_SPEED_MULTIPLIER)
            {
                ...
                if (currentSpeed > 0)
                {
                    ...
                    float newSpeed = std::sqrt(GameState::BALL_INITIAL_VELOCITY_X * GameState::BALL_INITIAL_VELOCITY_X + 
                                             GameState::BALL_INITIAL_VELOCITY_Y * GameState::BALL_INITIAL_VELOCITY_Y) * speedMultiplier;
